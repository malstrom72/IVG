...
- **Manifest generation and caching.**
  ...
  Collected files stream into an archive under the extension's `globalStorageUri`, and cached revisions prevent redundant base64 serialization when checksums remain unchanged so local edits stay snappy.
...
- **Webview bundle messaging.**
  ...
  The include workflow now emits a sibling `{ type: 'setIncludeBundle', revision, manifest, assets }` message where `assets` contains base64-encoded file data sourced directly from the local workspace, so the preview receives fresh resources without relying on an external upload target.【F:tools/ivg-vscode/src/extension.ts†L930-L1024】
  Because the payloads ride the existing VS Code messaging channel, the preview updates as soon as the manifest finishes building while the extension retains control over filesystem access and caching.【F:tools/ivg-vscode/src/extension.ts†L185-L404】【F:tools/ivg-vscode/src/extension.ts†L620-L828】
  The webview already emits `'status'` and `'trace'` packets that flow through `processStatusMessage` and `processTraceMessage`, so include bundle delivery inherits acknowledgment, error reporting, and timing hooks for free.【F:tools/ivg-vscode/src/extension.ts†L51-L157】【F:tools/ivg-vscode/src/extension.ts†L360-L475】
- **Editor-facing commands and UI.**
  The extension now contributes include-specific commands (`Show Include Assets`, `Rescan Include Assets`, open/reveal/copy mount path) alongside the preview commands so authors can drive the workflow from the command palette, the include explorer, or context menus without touching the codebase manually.【F:tools/ivg-vscode/package.json†L16-L115】
  A dedicated `TreeDataProvider` renders the **IVG Include Assets** view, wiring status summaries, asset rows, and context-menu handlers for opening files, revealing them on disk, or copying mount paths back into IMPD layouts.【F:tools/ivg-vscode/src/extension.ts†L557-L744】
  Companion settings such as `ivgfiddle.includes.autoRescan`, `ivgfiddle.includes.rescanDebounceMs`, and `ivgfiddle.includes.autoRevealExplorer` mirror the preview configuration structure so teams can opt into manual rescans, tweak debounce timing, or disable automatic explorer focus when manifests rebuild.【F:tools/ivg-vscode/src/extension.ts†L31-L127】【F:tools/ivg-vscode/package.json†L116-L168】
  A dedicated status bar affordance (`Includes: …`) tracks watcher availability, manifest progress, and manual-rescan warnings while exposing one-click navigation to the include explorer or the rescan command when manual intervention is required.【F:tools/ivg-vscode/src/extension.ts†L121-L205】【F:tools/ivg-vscode/src/extension.ts†L1224-L1293】
...
- **VS Code extension work.**
    - _File watching and manifest build pipeline:_ ~250–350 SLOC to add watcher registration, debounce helpers, manifest builders, and checksum caching. Expect 1.5–2.5 engineering days (or ~2–3 focused Codex coding sessions) given the need to thread async error handling through `queueMessage`, `syncDocument`, and the shared status infrastructure.【F:tools/ivg-vscode/src/extension.ts†L31-L475】
    - _Webview bundle messaging and telemetry:_ ~150–220 SLOC to craft the `{ type: 'setIncludeBundle' }` payloads, serialize assets into base64, and feed them through the existing `postMessageToWebview` throttle with status logging. Budget roughly 1 engineering day (1–2 Codex sessions) to iterate on serialization, caching, and UX polish.【F:tools/ivg-vscode/src/extension.ts†L185-L404】【F:tools/ivg-vscode/src/extension.ts†L930-L1024】
    - _Editor UI surfaces (tree view, commands, settings):_ ~200–280 SLOC across `extension.ts`, `package.json`, and new view/command modules. Another 1–1.5 engineering days for Codex to scaffold the `TreeDataProvider`, register commands, and localize strings.【F:tools/ivg-vscode/src/extension.ts†L215-L360】【F:tools/ivg-vscode/package.json†L1-L86】
- **IVGFiddle/webview integration.**
    - The preview now stages `{ type: 'setIncludeBundle' }` payloads, decodes base64 assets, and mounts them read-only under `/__ivg/includes` so synchronized resources survive renderer restarts without touching the host filesystem.【F:tools/ivg-vscode/media/previewShared.js†L1-L189】【F:tools/ivgfiddle/src/previewShared.js†L1-L210】
    - Cache invalidation happens automatically: each revision replaces the mounted archive atomically, the status bar logs the new revision, and the renderer re-rasterizes immediately so include edits propagate without manual refreshes.【F:tools/ivg-vscode/media/previewShared.js†L1750-L1905】【F:tools/ivgfiddle/src/previewShared.js†L1280-L1465】
    - When an include is missing or the renderer is offline, the webview emits `[IVGFiddle] Include missing` trace lines and status warnings, while the sandbox loader logs the same message after exhausting the synchronized archive, giving users actionable feedback instead of silent failures.【F:tools/ivg-vscode/media/previewShared.js†L1647-L1735】【F:tools/ivgfiddle/src/rasterizeIVG.cpp†L858-L910】
- **Total effort.** - Aggregating the above, anticipate 580–900 new or modified SLOC. A single developer (or Codex-assisted workflow) should expect roughly 4–6 engineering days end-to-end, including review cycles, integration testing, and UX polish.
