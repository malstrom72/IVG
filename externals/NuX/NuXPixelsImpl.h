...
template <typename T> bool Point<T>::operator==(const Point<T> &other) const {
  return (x == other.x && y == other.y);
}
template <typename T> bool Point<T>::operator!=(const Point<T> &other) const {
  return (x != other.x || y != other.y);
}
template <typename T> void Point<T>::swap(Point<T> &other) {
  std::swap(x, other.x);
  std::swap(y, other.y);
}
...
template <typename T> bool Rect<T>::isEmpty() const {
  return (width == 0 && height == 0);
}
template <typename T> bool Rect<T>::operator!=(const Rect<T> &other) const {
  return !(*this == other);
}
template <typename T> Rect<T> Rect<T>::offset(T x, T y) const {
  return Rect<T>(left + x, top + y, width, height);
}
...
template <typename T> Rect<T> Rect<T>::calcUnion(const Rect<T> &other) const {
  if (isEmpty()) {
    ...
    return Rect<T>(unionLeft, unionTop,
                   maxValue(calcRight(), other.calcRight()) - unionLeft,
                   maxValue(calcBottom(), other.calcBottom()) - unionTop);
  }
}

template <typename T>
Rect<T> Rect<T>::calcIntersection(const Rect<T> &other) const {
  T intersectionLeft = maxValue(left, other.left);
  T intersectionTop = maxValue(top, other.top);
  T intersectionWidth =
      minValue(calcRight(), other.calcRight()) - intersectionLeft;
  T intersectionHeight =
      minValue(calcBottom(), other.calcBottom()) - intersectionTop;
  if (intersectionWidth <= 0 || intersectionHeight <= 0) {
    return Rect<T>();
  } else {
    return Rect<T>(intersectionLeft, intersectionTop, intersectionWidth,
                   intersectionHeight);
  }
}
...
template <class T>
void standardFillPixels(int count, typename T::Pixel *target,
...
template <class T>
void fillPixels(int count, typename T::Pixel *target, typename T::Pixel color) {
  standardFillPixels<T>(count, target, color);
}

template <class T>
void standardCopyPixels(int count, typename T::Pixel *target,
                        const typename T::Pixel *source) {
  if (target != source) {
    int j = 0;
    while (j + 4 <= count) {
...
template <class T>
void copyPixels(int count, typename T::Pixel *target,
                const typename T::Pixel *source) {
  standardCopyPixels<T>(count, target, source);
}

template <class T>
void standardBlendSolidToPixels(int count, typename T::Pixel *target,
                                typename T::Pixel foreground,
...
template <class T>
void blendSolidToPixels(int count, typename T::Pixel *target,
                        typename T::Pixel foreground,
                        const typename T::Pixel *background) {
...
template <class T>
void standardBlendPixelsToPixels(int count, typename T::Pixel *target,
                                 const typename T::Pixel *foreground,
                                 const typename T::Pixel *background) {
  int i = 0;
  while (i + 4 <= count) {
    const typename T::Pixel a0 = background[i + 0];
...
template <class T>
void blendPixelsToPixels(int count, typename T::Pixel *target,
                         const typename T::Pixel *foreground,
                         const typename T::Pixel *background) {
...
template <class T>
inline void standardInterpolatePixels(int count, typename T::Pixel *&target,
                                      const typename T::Pixel *&source,
                                      int imageStride, Fixed32_32 &sx,
                                      Fixed32_32 &sy, const Fixed32_32 dxx,
                                      const Fixed32_32 dxy, int hop) {
  for (int i = 0; i < count; ++i) {
    *target = T::interpolate(source[0], source[1], source[imageStride],
                             source[imageStride + 1], low32(sx) >> 24,
                             low32(sy) >> 24);
...
template <class T>
void interpolatePixels(int count, typename T::Pixel *target,
                       const typename T::Pixel *source, int imageStride,
                       Fixed32_32 &sx, Fixed32_32 &sy, const Fixed32_32 dxx,
                       const Fixed32_32 dxy, int hop) {
  standardInterpolatePixels<T>(count, target, source, imageStride, sx, sy, dxx,
                               dxy, hop);
}

template <class T>
inline void
standardInterpolatePixelsXOnly(int count, typename T::Pixel *&target,
                               const typename T::Pixel *&source, Fixed32_32 &sx,
                               const Fixed32_32 dxx, int hop) {
  for (int i = 0; i < count; ++i) {
...
template <class T>
void interpolatePixelsXOnly(int count, typename T::Pixel *target,
                            const typename T::Pixel *source, Fixed32_32 &sx,
                            const Fixed32_32 dxx, int hop) {
...
template <class T>
inline void standardInterpolatePixelsYOnly(int count,
                                           typename T::Pixel *&target,
                                           const typename T::Pixel *&source,
                                           int imageStride, Fixed32_32 &sy,
                                           const Fixed32_32 dxy, int hop) {
  for (int i = 0; i < count; ++i) {
...
template <class T>
void interpolatePixelsYOnly(int count, typename T::Pixel *target,
                            const typename T::Pixel *source, int imageStride,
                            Fixed32_32 &sy, const Fixed32_32 dxy, int hop) {
  standardInterpolatePixelsYOnly<T>(count, target, source, imageStride, sy, dxy,
                                    hop);
}
...
template <>
void fillPixels<ARGB32>(int count, ARGB32::Pixel *target, ARGB32::Pixel color);
template <>
void copyPixels<ARGB32>(int count, ARGB32::Pixel *target,
                        const ARGB32::Pixel *source);
template <>
void blendSolidToPixels<ARGB32>(int count, ARGB32::Pixel *target,
                                ARGB32::Pixel foreground,
                                const ARGB32::Pixel *background);
template <>
void blendPixelsToPixels<ARGB32>(int count, ARGB32::Pixel *target,
                                 const ARGB32::Pixel *foreground,
                                 const ARGB32::Pixel *background);
template <>
void interpolatePixels<ARGB32>(int count, typename ARGB32::Pixel *target,
                               const typename ARGB32::Pixel *source,
                               int imageStride, Fixed32_32 &sx, Fixed32_32 &sy,
                               const Fixed32_32 dxx, const Fixed32_32 dxy,
                               int hop);
template <>
void interpolatePixelsXOnly<ARGB32>(int count, typename ARGB32::Pixel *target,
                                    const typename ARGB32::Pixel *source,
                                    Fixed32_32 &sx, const Fixed32_32 dxx,
                                    int hop);
template <>
void interpolatePixelsYOnly<ARGB32>(int count, typename ARGB32::Pixel *target,
                                    const typename ARGB32::Pixel *source,
                                    int imageStride, Fixed32_32 &sy,
                                    const Fixed32_32 dxy, int hop);
#endif
...
template <class T>
Span<T>::Span(int length, bool solid, bool opaque,
              const typename T::Pixel *pixels)
    : lengthAndFlags(length | (solid << 31) | (opaque << 30)), pixels(pixels) {
  assert(length >= 0);
}

template <class T> Span<T>::Span() {}
template <class T> bool Span<T>::isSolid() const {
  return ((lengthAndFlags & 0x80000000) != 0);
}
template <class T> bool Span<T>::isOpaque() const {
  return ((lengthAndFlags & 0x40000000) != 0);
}
template <class T> bool Span<T>::isTransparent() const {
  return (isSolid() && T::isTransparent(pixels[0]));
}
template <class T> bool Span<T>::isMaximum() const {
  return (isSolid() && T::isMaximum(pixels[0]));
}
template <class T> int Span<T>::getLength() const {
  return (lengthAndFlags & 0x3FFFFFFF);
}
template <class T> typename T::Pixel Span<T>::getSolidPixel() const {
  assert(isSolid());
  return pixels[0];
}
template <class T> const typename T::Pixel *Span<T>::getVariablePixels() const {
  assert(!isSolid());
  return pixels;
}
template <class T> const typename T::Pixel *Span<T>::getPixelPointer() const {
  return pixels;
}

/* --- SpanBuffer --- */

/** Iterator used to traverse spans stored inside a SpanBuffer */
template <class T> class SpanBuffer<T>::iterator {
public:
  iterator(Span<T> *p) : p(p) {}

public:
  Span<T> &operator*() { return *p; }
...
template <class T>
SpanBuffer<T>::SpanBuffer(char *spans, typename T::Pixel *pixels)
    : spans(reinterpret_cast<Span<T> *>(spans)), pixels(pixels),
      endSpan(reinterpret_cast<Span<T> *>(spans)), endPixel(pixels) {}
#elif (!NUXPIXELS_ICC_HACK)
template <class T>
SpanBuffer<T>::SpanBuffer(Span<T> *spans, typename T::Pixel *pixels)
    : spans(spans), pixels(pixels), endSpan(spans), endPixel(pixels) {}
#endif

template <class T>
typename SpanBuffer<T>::iterator SpanBuffer<T>::begin() const {
  return spans;
}
...
template <class T> void SpanBuffer<T>::addSpan(const Span<T> &span) {
  add(span.getLength(), span);
}
template <class T> void SpanBuffer<T>::addTransparent(int length) {
  addSolid(length, 0);
}

template <class T> typename T::Pixel *SpanBuffer<T>::preallocatePixels() const {
  return endPixel;
}

template <class T>
void SpanBuffer<T>::addSolid(int length, const typename T::Pixel &pixel) {
...
template <class T>
typename T::Pixel *SpanBuffer<T>::addVariable(int length, bool opaque) {
...
template <class T>
void SpanBuffer<T>::addReference(int length, const typename T::Pixel *pixels,
                                 bool opaque) {
#if !defined(NDEBUG)
...
template <class T>
void SpanBuffer<T>::split(
    iterator it,
    int splitPoint) // This method is a low-level method used for "merging" two
                    // SpanBuffers. It only splits for "forward iteration", but
                    // if you would iterate backwards the array would still be
                    // invariant since the split doesn't modify any pixel
                    // contents.
{
  ...
  if (solid) { // Solid pixel must be copied if it resides in this SpanBuffer.
               // Otherwise, it may be overwritten if we overwrite this buffer
               // forwards when merging.
...
template <class T>
Solid<T>::Solid(const typename T::Pixel &pixel) : pixel(pixel) {
  assert(T::isValid(pixel));
};
template <class T> IntRect Solid<T>::calcBounds() const { return FULL_RECT; }
template <class T>
void Solid<T>::render(int /*x*/, int /*y*/, int length,
...
template <class T>
SolidRect<T>::SolidRect(const typename T::Pixel &pixel, const IntRect &rect)
    : pixel(pixel), rect(rect) {
  assert(T::isValid(pixel));
}

template <class T> IntRect SolidRect<T>::calcBounds() const { return rect; }

template <class T>
void SolidRect<T>::render(int x, int y, int length,
                          SpanBuffer<T> &output) const {
...
template <class T>
Clipper<T>::Clipper(const Renderer<T> &source, const IntRect &rect)
    : source(source), rect(rect) {}
...
template <class T>
void Clipper<T>::render(int x, int y, int length, SpanBuffer<T> &output) const {
  // FIX : sub, or should we have a clipped-renderer class?
  assert(0 < length && length <= MAX_RENDER_LENGTH);
...
template <class T>
Offsetter<T>::Offsetter(const Renderer<T> &source, int offsetX, int offsetY)
    : source(source), offsetX(offsetX), offsetY(offsetY) {}
...
template <class T>
void Offsetter<T>::render(int x, int y, int length,
                          SpanBuffer<T> &output) const {
  assert(0 < length && length <= MAX_RENDER_LENGTH);
...
template <class T>
Raster<T>::Raster(typename T::Pixel *pixels, int stride, const IntRect &bounds,
                  bool opaque)
    : pixels(pixels), stride(stride), bounds(bounds), opaque(opaque) {}

template <class T> IntRect Raster<T>::calcBounds() const { return bounds; }

template <class T>
void Raster<T>::render(int x, int y, int length, SpanBuffer<T> &output) const {
  // FIX : sub, or should we have a clipped-renderer class?
  assert(0 < length && length <= MAX_RENDER_LENGTH);
...
template <class T>
void Raster<T>::fill(const Renderer<T> &source, const IntRect &area) {
  assert(area.isEmpty() ||
         bounds.calcUnion(area) ==
             bounds); // area must be within target raster bounds
...
template <class T> Raster<T> &Raster<T>::operator=(const Renderer<T> &source) {
  fill(source, bounds);
  return (*this);
}
template <class T> Raster<T> &Raster<T>::operator|=(const Renderer<T> &source) {
  fill((*this) | source, bounds.calcIntersection(source.calcBounds()));
  return (*this);
}
template <class T> Raster<T> &Raster<T>::operator+=(const Renderer<T> &source) {
  fill((*this) + source, bounds.calcIntersection(source.calcBounds()));
...
template <class T>
SelfContainedRaster<T>::SelfContainedRaster() : allocated(0) {
...
template <class T>
SelfContainedRaster<T>::SelfContainedRaster(const IntRect &bounds, bool opaque)
    : allocated(0) {
  this->bounds = bounds;
...
template <class T>
SelfContainedRaster<T>::SelfContainedRaster(const SelfContainedRaster &that)
    : allocated(0) {
  ...
  this->pixels =
      allocated - (this->bounds.top * this->stride + this->bounds.left);
  (*this) = static_cast<const Renderer<T> &>(that);
}

template <class T>
SelfContainedRaster<T> &
SelfContainedRaster<T>::operator=(const SelfContainedRaster &that) {
  if (&that != this) {
...
template <class T>
RLERaster<T>::RLERaster(const IntRect &bounds, const Renderer<T> &source)
    : bounds(bounds), opaque(false) {
  fill(source);
}
...
template <class T>
void RLERaster<T>::render(int x, int y, int length,
                          SpanBuffer<T> &output) const {
  // FIX : sub, or should we have a clipped-renderer class?
  assert(0 < length && length <= MAX_RENDER_LENGTH);
  ...
  if (y >= bounds.top && y < bounds.calcBottom()) {
    ...
    while (length > 0 && x < bounds.calcRight()) {
      ...
      if ((spans[spanIndex] & 0x8000) != 0) {
        ...
        output.addReference(c, &pixels[pixelIndex + x - sx],
                            ((spans[spanIndex] & 0x4000) != 0));
      }
      x += c;
      length -= c;
    }
...
template <class T> void RLERaster<T>::fill(const Renderer<T> &source) {
  ...
  for (int y = bounds.top; y < bottom; ++y) {
    newRLE.rows.push_back(
        std::pair<size_t, size_t>(newRLE.spans.size(), newRLE.pixels.size()));
    bool first = true;
    for (int x = bounds.left; x < right; x += MAX_RENDER_LENGTH) {
      ...
      while (it != output.end()) {
        ...
        UInt16 span = it->getLength() | (solidSpan ? 0x8000 : 0) |
                      (opaqueSpan ? 0x4000 : 0);
        if (!first && (span & 0xC000) == (newRLE.spans.back() & 0xC000) &&
            (!solidSpan || it->getSolidPixel() == newRLE.pixels.back()) &&
            ((newRLE.spans.back() & 0x3FFF) + it->getLength()) < 0x4000) {
          newRLE.spans.back() += it->getLength();
          if (!it->isSolid()) {
            newRLE.pixels.insert(newRLE.pixels.end(), it->getVariablePixels(),
                                 it->getVariablePixels() + it->getLength());
          }
        } else {
...
template <class T>
RLERaster<T> &RLERaster<T>::operator=(const Renderer<T> &source) {
  fill(source);
  return (*this);
}
template <class T>
...
UnaryOperator<S, T>::UnaryOperator(const Renderer<S> &source)
    : source(source) {}

template <class S, class T>
void UnaryOperator<S, T>::render(int x, int y, int length,
                                 SpanBuffer<S> &inputBuffer,
                                 SpanBuffer<T> &output) const {
...
template <class S, class T>
template <class U>
void UnaryOperator<S, T>::render(int x, int y, int length, const Renderer<U> &,
                                 SpanBuffer<T> &output) const {
...
template <class S, class T>
void UnaryOperator<S, T>::render(int x, int y, int length, const Renderer<T> &,
                                 SpanBuffer<T> &output) const {
...
template <class T> LookupTable<T>::LookupTable() : opaque(false) {}

template <class T>
LookupTable<T>::LookupTable(const typename T::Pixel *table, bool opaque)
    : opaque(opaque) {
...
Lookup<T, L>::Lookup(const Renderer<Mask8> &source, const L &table)
    : UnaryOperator<Mask8, T>(source), table(table) {}
...
template <class T, class L>
void Lookup<T, L>::process(int count, const Mask8::Pixel *source,
...
template <class T>
Inverter<T>::Inverter(const Renderer<T> &source)
    : UnaryOperator<T, T>(source) {}

template <class T> IntRect Inverter<T>::calcBounds() const { return FULL_RECT; }

template <class T>
void Inverter<T>::process(int count, const typename T::Pixel *source,
                          typename T::Pixel *target, bool &opaque) const {
  for (int i = 0; i < count; ++i)
    target[i] = T::invert(source[i]);
  opaque = false;
}
...
template <class S, class T>
Converter<S, T>::Converter(const Renderer<S> &source)
    : UnaryOperator<S, T>(source) {}

template <class S, class T> IntRect Converter<S, T>::calcBounds() const {
  return T::isTransparent(convert(S(), T(), S::transparent()))
             ? super::source.calcBounds()
             : FULL_RECT;
}

template <class S, class T>
void Converter<S, T>::process(int count, const typename S::Pixel *source,
                              typename T::Pixel *target,
                              bool & /*opaque*/) const {
  for (int i = 0; i < count; ++i)
    target[i] = convert(S(), T(), source[i]);
}

/* --- BinaryOperator --- */

template <class A, class B>
BinaryOperator<A, B>::BinaryOperator(const Renderer<A> &rendererA,
...
template <class T>
Blender<T>::Blender(const Renderer<T> &rendererA,
                    const Renderer<T> &rendererB) ///< rendererA is background,
                                                  ///< rendererB is overlay
    : super(rendererA, rendererB), boundsA(rendererA.calcBounds()),
...
template <class T>
void Blender<T>::render(int x, int y, int length, SpanBuffer<T> &output) const {
  // FIX : Example of unoptimized code in comment here...

  assert(0 < length && length <= MAX_RENDER_LENGTH);

  bool intersectsA = y >= boundsA.top && y <= boundsA.top + boundsA.height &&
                     x + length > boundsA.left &&
                     x < boundsA.left + boundsA.width;
  bool intersectsB = y >= boundsB.top && y <= boundsB.top + boundsB.height &&
                     x + length > boundsB.left &&
                     x < boundsB.left + boundsB.width;
  ...
  while (itA != spansA.end()) {
    ...
    if (spanB.isTransparent()) {
      ...
      output.addSolid(spanLength,
                      T::blend(spanA.getSolidPixel(), spanB.getSolidPixel()));
    } else {
      typename T::Pixel *pixels =
          output.addVariable(spanLength, spanA.isOpaque());
      if (spanA.isSolid()) {
        ...
        blendSolidToPixels<T>(spanLength, pixels, spanB.getSolidPixel(),
                              spanA.getVariablePixels());
      } else {
        blendPixelsToPixels<T>(spanLength, pixels, spanB.getVariablePixels(),
                               spanA.getVariablePixels());
      }
    }
  }
...
template <class T>
Adder<T>::Adder(const Renderer<T> &rendererA, const Renderer<T> &rendererB)
...
template <class T>
void Adder<T>::render(int x, int y, int length, SpanBuffer<T> &output) const {
  assert(0 < length && length <= MAX_RENDER_LENGTH);
  ...
  while (itA != spansA.end()) {
    ...
    if (spanA.isMaximum() || spanB.isTransparent()) {
      ...
      output.addSolid(spanLength,
                      T::add(spanA.getSolidPixel(), spanB.getSolidPixel()));
    } else {
      typename T::Pixel *pixels =
          output.addVariable(spanLength, spanA.isOpaque() || spanB.isOpaque());
      if (spanA.isSolid()) {
        typename T::Pixel pixelA = spanA.getSolidPixel();
        const typename T::Pixel *pixelsB = spanB.getVariablePixels();
        for (int i = 0; i < spanLength; ++i)
          pixels[i] = T::add(pixelA, pixelsB[i]);
      } else if (spanB.isSolid()) {
        const typename T::Pixel *pixelsA = spanA.getVariablePixels();
        typename T::Pixel pixelB = spanB.getSolidPixel();
        for (int i = 0; i < spanLength; ++i)
          pixels[i] = T::add(pixelsA[i], pixelB);
      } else {
        const typename T::Pixel *pixelsA = spanA.getVariablePixels();
        const typename T::Pixel *pixelsB = spanB.getVariablePixels();
        for (int i = 0; i < spanLength; ++i)
          pixels[i] = T::add(pixelsA[i], pixelsB[i]);
      }
    }
  }
...
template <class A, class B>
Multiplier<A, B>::Multiplier(const Renderer<A> &rendererA,
                             const Renderer<B> &rendererB)
    : super(rendererA, rendererB) {}

template <class A, class B> IntRect Multiplier<A, B>::calcBounds() const {
  return super::rendererA.calcBounds().calcIntersection(
      super::rendererB.calcBounds());
}

template <class A, class B>
void Multiplier<A, B>::render(int x, int y, int length,
                              SpanBuffer<A> &output) const {
  assert(0 < length && length <= MAX_RENDER_LENGTH);
...
template <class T>
Optimizer<T>::Optimizer(const Renderer<T> &source) : source(source) {}
...
template <class T>
const typename T::Pixel *
Optimizer<T>::outputVariable(const typename T::Pixel *b,
                             const typename T::Pixel *e, bool opaque,
                             SpanBuffer<T> &output) {
  if (e - b != 0) {
...
template <class T>
const typename T::Pixel *Optimizer<T>::analyzeSolid(const typename T::Pixel *b,
...
template <class T>
const typename T::Pixel *Optimizer<T>::analyzeOpaque(const typename T::Pixel *b,
                                                     const typename T::Pixel *e,
                                                     SpanBuffer<T> &output) {
...
template <class T>
const typename T::Pixel *
Optimizer<T>::analyzeNonOpaque(const typename T::Pixel *b,
                               const typename T::Pixel *e,
                               SpanBuffer<T> &output) {
  const typename T::Pixel *p = b;
  while (p != e) {
    ...
    } else if (p + 4 <= e && T::isOpaque(p[0]) && T::isOpaque(p[1]) &&
               T::isOpaque(p[2]) && T::isOpaque(p[3])) {
...
template <class T>
void Optimizer<T>::render(int x, int y, int length,
                          SpanBuffer<T> &output) const {
  assert(0 < length && length <= MAX_RENDER_LENGTH);
  ...
  while (it != inputBuffer.end()) {
    if (it->isSolid()) {
      ...
      assert(b == e); // If not we might have run into an unopaque pixel in an
                      // opaque span.
    }
  }
}
  ...
  for (int tableIndex = 0; tableIndex < 256; ++tableIndex) {
    while (tableIndex >= positions[pointIndex + 1]) {
      ...
      if (positions[pointIndex + 1] > positions[pointIndex]) {
        fractionScale =
            (1 << 30) / (positions[pointIndex + 1] - positions[pointIndex]);
      }
    }
    UInt32 fraction = static_cast<UInt32>(
        (tableIndex - positions[pointIndex]) * fractionScale >> 22);
    assert(0 <= fraction && fraction <= 256);
    super::table[tableIndex] =
        T::interpolate(colors[pointIndex], colors[pointIndex + 1], fraction);
...
template <class T>
Gradient<T>::Gradient(typename T::Pixel start, typename T::Pixel end) {
...
/**
        Impl holds the implementation details of Texture such as sampling and
...
public:
  Impl(const Raster<T> &image, bool wrap,
       const AffineTransformation &transformation, const IntRect &sourceRect);

protected:
  int findImage(int span, Fixed32_32 &sx, Fixed32_32 &sy,
                SpanBuffer<T> &output) const;

protected:
  int interpolateEdge(int span, Fixed32_32 &sx, Fixed32_32 &sy,
                      SpanBuffer<T> &output) const;

protected:
  int interpolateInside(int span, Fixed32_32 &sx, Fixed32_32 &sy,
                        SpanBuffer<T> &output) const;

protected:
  void calcStartPosition(int x, int y, Fixed32_32 &sx, Fixed32_32 &sy) const;
...
protected:
  enum TransformType {
    ...
    ,
    FRACTIONAL_X ///< Fractional horizontal translation, interpolate
                 ///< horizontally only.
    ,
    FRACTIONAL_Y ///< Fractional vertical translation, interpolate vertically
                 ///< only.
    ,
...
template <class T>
Texture<T>::Impl::Impl(const Raster<T> &image, bool wrap,
    ...
    : imageBounds(image.calcBounds().calcIntersection(sourceRect)),
      ...
      imagePixels(
          image.getPixelPointer() + imageBounds.top * imageStride +
          imageBounds
              .left) // Offset image so that 0, 0 is always top-left coordinate.
      ,
      opaque(image.isOpaque()), wrap(wrap),
  ...
  // If not inversible, this probably means an extremely small scale, so no need
  // to draw anything.

  if (inverseXForm.invert()) {
    // If not wrapping, find approximative boundaries, to speed up further
    // rendering.

    if (!wrap) {
      // FIX : optimize, is there an efficient formula for this?
      Path p;
      p.addRect(imageBounds.left - 1, imageBounds.top - 1,
                imageBounds.width + 1, imageBounds.height + 1);
    ...
    // We offset with the image top-left in the "integer world" so that any
    // source clipping will be exact.

    ox = add(toFixed32_32(inverseXForm.matrix[0][2]),
             toFixed32_32(-imageBounds.left, 0));
    oy = add(toFixed32_32(inverseXForm.matrix[1][2]),
             toFixed32_32(-imageBounds.top, 0));
    hop = high32(dxy) * imageStride + high32(dxx);

    // Find optimal transformation algorithm.

    bool horizontalInterpolation =
        (low32(dxx) != 0 || low32(dyx) != 0 || (low32(ox) >> 24) != 0);
    bool verticalInterpolation =
        (low32(dxy) != 0 || low32(dyy) != 0 || (low32(oy) >> 24) != 0);
    bool noInterpolation = (!horizontalInterpolation && !verticalInterpolation);

    if (high32(dxx) == 1 && high32(dxy) == 0 && high32(dyx) == 0 &&
        high32(dyy) == 1 && noInterpolation) {
      transformType = IDENTITY;
    } else if (noInterpolation) {
      transformType = INTEGER;
    } else if (high32(dxx) >= -1 && high32(dxx) <= 0 && high32(dxy) == 0 &&
               low32(dxy) == 0) {
...
template <class T>
int Texture<T>::Impl::findImage(int length, Fixed32_32 &sx, Fixed32_32 &sy,
                                SpanBuffer<T> &output) const {
  assert(length > 0);

  if (wrap) {
    sx =
        toFixed32_32(NuXPixels::wrap(high32(sx), imageBounds.width), low32(sx));
    sy = toFixed32_32(NuXPixels::wrap(high32(sy), imageBounds.height),
                      low32(sy));
    return 0;
  } else {
    // Binary search for the optimal run-length until image begins (this is
    // similar to a standard division algorithm).
    ...
    for (int shift = MAX_SPAN_BITS; shift >= 0; --shift) {
      ...
      if ((col < -1 && high32(nx) < -1) ||
          (col >= imageBounds.width && high32(nx) >= imageBounds.width) ||
          (row < -1 && high32(ny) < -1) ||
          (row >= imageBounds.height && high32(ny) >= imageBounds.height)) {
        spanLength += (1 << shift);
...
template <class T>
int Texture<T>::Impl::interpolateEdge(int length, Fixed32_32 &sx,
                                      Fixed32_32 &sy,
                                      SpanBuffer<T> &output) const {
  assert(length > 0);
  ...
  do {
    ...
    if (wrap) {
      ...
      int y1 = ((row + 1 < imageBounds.height)
                    ? imageStride
                    : imageStride - imageBounds.height * imageStride);
      c00 = s[x0 + y0];
      c10 = s[x1 + y0];
      c01 = s[x0 + y1];
      c11 = s[x1 + y1];
      ...
      c11 = (col + 1 < imageBounds.width && row + 1 < imageBounds.height)
                ? s[imageStride + 1]
                : 0;
    }
    ...
    } while (high32(sx) == col && high32(sy) == row &&
             d < e); // Note: can't just check delta, cause we can actually land
                     // at the same sample offset even if we change row and
                     // column (e.g. in a 1*1 image).

    s += delta;
    col = high32(sx);
    row = high32(sy);

  } while (d < e && col >= -1 && col < imageBounds.width && row >= -1 &&
           row < imageBounds.height &&
           (col == -1 || col == imageBounds.width - 1 || row == -1 ||
            row == imageBounds.height - 1));

  typename T::Pixel *checkPixels =
      output.addVariable(static_cast<int>(d - pixels), wrap && opaque);
  (void)checkPixels;
...
template <class T>
int Texture<T>::Impl::interpolateInside(int length, Fixed32_32 &sx,
                                        Fixed32_32 &sy,
                                        SpanBuffer<T> &output) const {
  assert(length > 0);
  ...
  if (transformType == IDENTITY) {
    ...
    // Binary search for the optimal run-length until image wraps / ends (this
    // is similar to a standard division algorithm).
    ...
    for (int shift = MAX_SPAN_BITS; shift >= 0; --shift) {
      ...
      if (high32(nx) >= 0 && high32(nx) + 1 < imageBounds.width &&
          high32(ny) >= 0 && high32(ny) + 1 < imageBounds.height) {
  ...
  const typename T::Pixel *s =
      &imagePixels[high32(sy) * imageStride + high32(sx)];
  ...
  case UPSCALE: {
    ...
    while (i < spanLength) {
      ...
      typename T::Pixel argb0 =
          T::interpolate(s[0], s[imageStride], rowFraction);
      typename T::Pixel argb1 =
          T::interpolate(s[1], s[imageStride + 1], rowFraction);
      int delta;
      do {
  ...
  case ARBITRARY: {
    typename T::Pixel *pixels = output.addVariable(spanLength, opaque);
    assert(high32(sx) >= 0 && high32(sx) + 1 < imageBounds.width &&
           high32(sy) >= 0 && high32(sy) + 1 < imageBounds.height);
    interpolatePixels<T>(spanLength, pixels, s, imageStride, sx, sy, dxx, dxy,
                         hop);
  } break;
...
template <class T>
void Texture<T>::Impl::calcStartPosition(int x, int y, Fixed32_32 &sx,
                                         Fixed32_32 &sy) const {
  switch (transformType) {
  case IDENTITY: {
  ...
  case ARBITRARY: {
    ...
    sx = add(add(ox, multiply(static_cast<UInt32>(x), dxx)),
             multiply(static_cast<UInt32>(y), dyx));
    sy = add(add(oy, multiply(static_cast<UInt32>(x), dxy)),
             multiply(static_cast<UInt32>(y), dyy));
  } break;
...
template <class T>
Texture<T>::Texture(const Raster<T> &image, bool wrap,
                    const AffineTransformation &transformation,
                    const IntRect &sourceRect)
...
template <class T>
void Texture<T>::render(int x, int y, int length, SpanBuffer<T> &output) const {
  assert(0 < length && length <= MAX_RENDER_LENGTH);

  // Invalid transform (probably too small) or outside outputBounds (if not
  // wrapping)

  if (impl->transformType == Impl::INVALID || y < impl->outputBounds.top ||
      y >= impl->outputBounds.calcBottom() ||
      x + length <= impl->outputBounds.left ||
      x >= impl->outputBounds.calcRight()) {
    output.addTransparent(length);
    return;
  }
  ...
  */
  ...
  case Impl::FRACTIONAL_X:
    rowMargin = 0;
    break;
  case Impl::FRACTIONAL_Y:
    colMargin = 0;
    break;
  default:
    break;
  ...
  while (offset < length) {
    ...
    int row = high32(sy);
    int spanLength;
    if (col < -colMargin || col >= impl->imageBounds.width ||
        row < -rowMargin || row >= impl->imageBounds.height) {
      spanLength = impl->findImage(length - offset, sx, sy, output);
    } else if (col < 0 || col + colMargin >= impl->imageBounds.width ||
               row < 0 || row + rowMargin >= impl->imageBounds.height) {
...
template <class T> Texture<T>::~Texture() { delete impl; }
