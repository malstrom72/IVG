...
A directive may also supply an optional `common:[ ... ]` block. The shared statements execute before every scenario entry inside the directive, including rounds where the executor skips per-scenario bodies because they belong to another selection. Shared-only directives are valid and simply perform setup without contributing additional entries to the plan. Common blocks are compared across iterative runs just like scenario bodies, so changes between rounds trigger syntax errors instead of silently diverging.
